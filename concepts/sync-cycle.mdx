...
description: "Understand the upstream sync behind the Trassets Data API: TRUNCATE+Reload semantics, last_sync_at, and what GET /changes actually detects."
---

The Trassets Data API is a read-only interface over data that is synced in from an
upstream pipeline. Understanding how and when that sync happens is essential for
building a correct polling or incremental-sync integration.

<Note>
  The sync cadence is a property of the upstream pipeline, not something the API
  configures or exposes — it has no machine-enforced schedule of its own to verify
  against. Do not assume a fixed refresh interval; always check `last_sync_at` (see
  below) instead.
</Note>
...
`NULL` until the first successful sync, and a failed or in-progress sync does not advance
it. This means:

- A table with no successful sync yet will never appear in `GET /changes`, regardless of
  how recently a sync was attempted.
...
`GET /changes?since=<timestamp>` returns a table only if it has completed a successful sync
with `last_sync_at > since` — a **strict** greater-than comparison, not `>=`.
...
<Warning>
  ...
  limit is documented in the endpoint's OpenAPI parameter description (see the **API
  Reference** tab), but no rationale for the specific figure is given anywhere; treat it as
  a fixed operational constraint rather than a tunable setting.
</Warning>
