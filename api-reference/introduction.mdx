...
The API currently lives at the base path. URL versioning under `/v1/` is planned but not enforced yet. Once `/v1/` ships, the unversioned base path keeps working for a limited deprecation window before it is sunset — see [Versioning](/concepts/versioning) for the full policy. Until versioning is active, breaking changes may occur. We recommend pinning your integration to the documented behavior and monitoring the changelog for updates.
