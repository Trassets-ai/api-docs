---
title: "Data API Reference: Endpoints Overview"
sidebarTitle: "Introduction"
description: "Overview of the Data API: base URL, authentication, endpoint types, pagination, errors, and versioning."
---

The Data API is a REST interface for real-estate and market data. Base URL: `https://api.trassets.ai`. Send your API key in the `X-API-Key` header on every request. All responses are JSON unless noted otherwise.
...
Tables are migrated to `/v1/` one at a time, alongside the unversioned base path — both work today, with no redirect and no confirmed sunset date for unversioned paths yet. See [Versioning](/concepts/versioning) for the current rollout state and how to migrate incrementally. On base paths without a `/v1/` counterpart, breaking changes may still occur.
