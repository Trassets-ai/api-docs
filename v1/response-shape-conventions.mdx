...
| Verified sync-pipeline join keys are renamed, not dropped | `"recommended_join_key": "1204"` | `"property_structure_account_id": "1204"` — kept once verified as a real foreign key (e.g. on `booking_positions`, `booking_header`, `technical_objects`) |
| Unverifiable sync-pipeline artifacts are dropped | `"property_id_account_number": 1101410500` | removed entirely — join on the real ID fields instead |
...
<Note>
  These conventions are defined in an internal architecture decision record (status: Accepted) and are still applied table by table — most `/v1/` tables don't fully follow them yet.
</Note>
