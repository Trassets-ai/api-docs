...
<ResponseField name="property_structure_account_id" type="string">
  Foreign key into `Property.property_structure`, matching its `account_id`. Populated
  conditionally: set only for rows without a `floor_area_id`; everywhere else it's `null` and
  `floor_area_id` is the join key. The two never appear together and never both `null`.
</ResponseField>
