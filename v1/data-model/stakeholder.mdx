...
<Note>
  `person_id` isn't type-consistent across the endpoints that carry it: `tenants.person_id`
  and `Accounting.booking_header.person_id` are strings (e.g. `"2001-1"`), but
  `Accounting.booking_positions.person_id` is an integer with the hyphen stripped — casting
  it back to a string doesn't reproduce the original value. Don't join `booking_positions`
  to `tenants` directly on `person_id`; go via `booking_positions.booking_header_id` →
...
<ResponseField name="exemption_certificate_date" type="date">
  Date on the exemption certificate.
</ResponseField>
