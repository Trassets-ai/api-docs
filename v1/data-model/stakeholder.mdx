...
<Note>
  ...
  `property_id` plus `person_id` can return more than one row per person. `tenancy_id` is also
  what [`Contracts.contract_debits` and `Contracts.contract_ends`](/v1/data-model/contracts)
  reference.
</Note>
