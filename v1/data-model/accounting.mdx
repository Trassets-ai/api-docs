...
<ResponseField name="property_structure_account_id" type="string">
  ...
  Always populated, and the only way to resolve a booking down to a single unit —
  `booking_header` carries no `floor_area_id`. `property_structure` has no `/v1/` endpoint
  yet, so fetch it via the unversioned path.
</ResponseField>
