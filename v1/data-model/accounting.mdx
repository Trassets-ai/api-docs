...
<Note>
  `nr` is `null` for accounts still on the older account-range classification — that
  classification doesn't produce this field. `category` and `group` are still populated in
  that case, derived differently under the hood.
</Note>
