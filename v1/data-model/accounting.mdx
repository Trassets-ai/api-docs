...
`booking_positions` also carries a `person_id`, but as `integer` rather than the `string`
used by `booking_header.person_id` and `tenants.person_id` — it doesn't join directly to
`tenants`. To link a posting to a tenant, go through `booking_positions.booking_header_id`
to `booking_header.person_id` instead.
