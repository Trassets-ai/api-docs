...
<Note>
  `kontenmapping_ranges` is not getting a `/v1/` endpoint — it stays available as
  [`Accounting.kontenmapping_ranges`](/data-model/accounting#kontenmapping_ranges) on the
  unversioned path.
</Note>
