...
<ResponseField name="account_number" type="string">
  Debit-type code the deviation applies to. Despite the field name, this is not a ledger
  account reference — it matches `contract_debits.debit_type.code`, not
  `Accounting.accounts`.
</ResponseField>
...
<ResponseField name="debit_type" type="object">
  ...
  <Expandable title="properties">
    <ResponseField name="code" type="integer">
      Debit-type code. Matches `contract_debit_diffs.account_number` — not a ledger account
...
<Warning>
  There is no stable ID on this endpoint — `floor_area_person_id` alone does not uniquely
  identify a row, since a tenancy can have multiple option/extension entries. Treat rows as
  a set per `floor_area_person_id`, not as individually addressable records.
</Warning>
...
<Warning>
  `min_date`/`max_date` and `begin_contract`/`end_contract` are two more date-range pairs
  alongside `validity_period`, kept separate deliberately. Their exact relationship to
  `validity_period` and to each other has not been confirmed against the source system —
  don't assume any of the three pairs are interchangeable.
</Warning>
