---
title: "v1 Data Model"
sidebarTitle: "Overview"
description: "The tables available under /v1/ and how they join — Accounting, Contracts, Depreciation, Operations, Property, and Stakeholder."
---

`/v1/` covers six data domains. Each domain page holds the field reference for its tables.

<CardGroup cols={2}>
  <Card title="Accounting" icon="book" href="/v1/data-model/accounting">
    `accounts`, `booking_header`, `booking_positions`, and `budgets`, with `period`, `amount`, and `amount_per_sqm` objects.
  </Card>

  <Card title="Contracts" icon="file-contract" href="/v1/data-model/contracts">
    `service_contracts`, with the `contract_period` object.
  </Card>

  <Card title="Depreciation" icon="chart-line-down" href="/v1/data-model/depreciation">
    `assets`, with the `afa_period` object and the `asset_id`/`asset_number` pair.
  </Card>

  <Card title="Operations" icon="clipboard-list" href="/v1/data-model/operations">
    `notifications` and `projects`, with `technical_object_id` and the `hierarchy` array.
  </Card>

  <Card title="Property" icon="building" href="/v1/data-model/property">
    `properties`, `property_creditor_services`, and `technical_objects`, with the `entrances` array, `mandate_period` object, and conditional `property_structure_account_id` fallback.
  </Card>

  <Card title="Stakeholder" icon="users" href="/v1/data-model/stakeholder">
    `creditor` and `tenants`, both with a `phones` array.
  </Card>
</CardGroup>

<Note>
  These tables have no `/v1/` endpoint yet: `Accounting.kontenmapping_ranges`,
  `Contracts.contract_debits`/`contract_debit_diffs`/`contract_ends`,
  `Depreciation.depreciation` (the postings table — `assets` is available, the postings
  aren't), `Operations.offers`/`orders`, `Property.property_structure`, and
  `Stakeholder.employees`/`owner`. Fetch them via the unversioned path in the meantime — see
  the [full data model](/data-model/overview).
</Note>

## How the v1 tables connect

`property_id` is the join key shared across every table on this page.

```mermaid
graph LR
  properties["Property.properties<br/>(property_id)"]
  creditor["Stakeholder.creditor<br/>(creditor_id)"]

  properties -->|property_id| booking_header["Accounting.booking_header"]
  properties -->|property_id| notifications["Operations.notifications"]
  properties -->|property_id| projects["Operations.projects"]
  properties -->|property_id| technical_objects["Property.technical_objects<br/>(to_id)"]
  properties -->|property_id| property_creditor_services["Property.property_creditor_services"]
  properties -->|property_id| budgets["Accounting.budgets"]
  properties -->|property_id| tenants["Stakeholder.tenants"]
  properties -->|property_id| service_contracts["Contracts.service_contracts"]
  properties -->|property_id| assets["Depreciation.assets"]

  booking_header -->|booking_header_id| booking_positions["Accounting.booking_positions"]
  booking_positions -->|account_id| accounts["Accounting.accounts"]
  budgets -->|account_id| accounts
  notifications -->|technical_object_id| technical_objects
  notifications -->|report_id| projects
  technical_objects -->|service_contract_id| service_contracts
  booking_header -->|property_id + person_id| tenants
  booking_positions -->|property_id + person_id| tenants

  creditor -->|creditor_id| technical_objects
  creditor -->|creditor_id| property_creditor_services
  creditor -->|creditor_id| service_contracts
  creditor -->|creditor_id| booking_header
  creditor -->|creditor_id| booking_positions
```

### Join keys in detail

| Key | Meaning | Cardinality |
|---|---|---|
| `property_id` | Identifies a property. Present on every table on this page except `creditor` (a standalone entity, referenced via `creditor_id` instead). | 1 property → many rows in each domain |
| `booking_header_id` | Groups booking line items under one header. | 1 `booking_header` → many `booking_positions` |
| `account_id` | Internal account identifier. | 1 `accounts` row → many `booking_positions`/`budgets` rows |
| `creditor_id` | Identifies a creditor. Referenced by `technical_objects`, `property_creditor_services`, `service_contracts`, `booking_header`, and `booking_positions` — unreliable on `projects`, see below. | 1 creditor → many rows across domains |
| `technical_object_id` | Links a notification to the technical object it concerns. References `technical_objects.to_id`. | 1 technical object → many notifications |
| `report_id` | Links a project back to the notification it originated from, if any. | 1 notification → 0..N projects |
| `service_contract_id` | Links a technical object to the service contract covering it. References `service_contracts.contract_id`. | 1 contract → many technical objects |
| `person_id` | Links a booking to the tenant it applies to. Only valid combined with `property_id`, since the numbering restarts per property. | 1 person → 1..N tenancies per property |
| `property_structure_account_id` | Foreign key into `Property.property_structure` (available via the unversioned path), present on `booking_header`, `booking_positions`, and `technical_objects`. Always populated on the two booking tables, where it's the only route from a booking down to a unit; on `technical_objects` it's the fallback for rows without a `floor_area_id`. | 1 unit → many rows |

<Note>
  Cardinalities above describe the intended data model, not independently verified row
  counts. `projects.creditor_id` and `projects.to_hdr_id` are deliberately left out of this
  diagram — both are flagged as unreliable in the [Operations field
  reference](/v1/data-model/operations), so don't treat them as working joins.
</Note>

<Warning>
  `property_id` and `creditor_id` come back as JSON strings on most endpoints but as numbers
  on `booking_positions` and `projects`, and `person_id` is a string on `tenants` and
  `booking_header` but a number on `booking_positions`. Cast to a common type before joining
  across those boundaries — a strict equality check on the raw values silently returns no
  rows.
</Warning>
