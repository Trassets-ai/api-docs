...
post to — as `ledger_id` and `account_number` respectively — again matching
`accounts.account_number`. `contract_debits.debit_type.code` is a different thing, a
debit-type code, not a ledger reference (see the `account_id` row below).
...
| `ledger_id` | General ledger account a debit line posts to. Named `ledger_id` on `contract_debits`, `account_number` on `contract_debit_diffs` — same target, different field name on each. Matches `accounts.account_number`, not `accounts.account_id`. | 1 `accounts` row → many `contract_debits`/`contract_debit_diffs` rows |
