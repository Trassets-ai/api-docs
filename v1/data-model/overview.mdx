...
| `floor_area_person_id` | Identifies a tenancy. Shared by `tenants`, `contract_debits`, `contract_ends`, and `index_rent` as the same underlying value — join across them directly, without `property_id`. An integer everywhere except `contract_debits`, where it's a string; cast before joining. | 1 tenancy → many debit/end/index rows |
...
<Warning>
  ...
  `booking_header_id` and use the header's `person_id`. `floor_area_id` and
  `floor_area_person_id` have a milder version of the same problem: a number everywhere except
  `contract_debits`, where they're strings, but there a plain cast does work.
</Warning>
