...
<CardGroup cols={2}>
  ...
  <Card title="Contracts" icon="file-contract" href="/v1/data-model/contracts">
    `service_contracts`, `contract_debits`, `contract_debit_diffs`, and `contract_ends`, with the `contract_period`, `debit_period`, `debit_type`, `period`, and `option_state` objects.
  </Card>
...
| `person_id` | Links a booking to the tenant it applies to. Only valid combined with `property_id`, since the numbering restarts per property. | 1 person → 1..N tenancies per property |
| `tenancy_id` | Identifies a tenancy. Shared by `tenants`, `contract_debits`, and `contract_ends` as the same underlying value — join across them directly, without `property_id`. | 1 tenancy → many debit/end rows |
...
<Warning>
  ...
  `booking_positions`. Cast to a common type before joining across that boundary — a strict
  equality check on the raw values silently returns no rows. `floor_area_id` has the same
  problem in reverse: a number everywhere except `contract_debits`, where it's a string.
</Warning>
