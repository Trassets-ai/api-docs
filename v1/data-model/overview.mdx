...
`contract_debits` and `contract_debit_diffs` both reference the general ledger account they
post to, via `account_number` on each — matching `accounts.account_number`.
`contract_debits.debit_type.code` is a different thing, a debit-type code, not a ledger
reference (see the `account_id` row below).
...
| `account_id` | Internal account identifier on `accounts`, `booking_positions`, and `budgets` — joins across those three. `index_rent.account_id` and `contract_debits.debit_type.code` mean something unrelated: a debit-type code — don't join those to `accounts`. On `property_structure`, it means something else again: a cost-centre business code, matching `accounts.account_number` for the corresponding cost-centre row, not `accounts.account_id`. | 1 `accounts` row → many `booking_positions`/`budgets` rows |
| `account_number` (on `contract_debits`/`contract_debit_diffs`) | General ledger account a debit line posts to — on both `contract_debits` and `contract_debit_diffs`. Matches `accounts.account_number`, not `accounts.account_id`. | 1 `accounts` row → many `contract_debits`/`contract_debit_diffs` rows |
...
| `technical_object_id` | Present on `notifications` and `orders`, resolves against `technical_objects.technical_object_id` — same field name on both sides, but not the same reliability: ~10% of `notifications` rows and ~25% of `orders` rows don't resolve. | 1 technical object → many notifications |
