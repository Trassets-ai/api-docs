...
description: "The tables currently available under /v1/, reshaped per the response shape conventions, and how they join — Accounting, Contracts, Depreciation, Operations, Property, and Stakeholder."
---

`/v1/` only documents a table once it's live. All six domains now have at least one
documented table.
...
<CardGroup cols={2}>
  ...
  <Card title="Depreciation" icon="chart-line-down" href="/v1/data-model/depreciation">
    `assets`, with the `afa_period` object and the renamed `asset_id`/`asset_number` pair.
  </Card>
...
<Note>
  The rest of the unversioned data model isn't live under `/v1/` yet:
  `Accounting.kontenmapping_ranges`, all of `Contracts.contract_debits`/
  `contract_debit_diffs`/`contract_ends`, `Depreciation.depreciation` (the postings table —
  `assets` is live, the postings aren't), `Operations.offers`/`orders`,
  `Property.property_structure`, and `Stakeholder.employees`/`owner`. Use their unversioned
  equivalents in the [full data model](/data-model/overview) for now.
</Note>
...
| `account_id` | Internal account identifier, renamed from the unversioned `account_hdr_id`. | 1 `accounts` row → many `booking_positions`/`budgets` rows |
