...
<Note>
  `Operations.offers` and `orders` have no `/v1/` endpoint yet — fetch them via the
  unversioned path in the meantime. `Accounting.kontenmapping_ranges` is not getting one at
  all and stays on the unversioned path permanently. All three are documented in the
...
Verified against live data, holding for every row where the key is populated:

| Rule | Holds on |
|---|---|
| `floor_area_id` starts with `property_id` | `property_structure`, `tenants`, `contract_debits`, `contract_ends` |
| `floor_area_person_id` starts with `floor_area_id` | `tenants`, `contract_debits`, `contract_ends` |
| `property_id_person_id` starts with `property_id` | `contract_debits`, `booking_positions` — null there on rows with no person, never mismatched |
| `property_structure.account_id` starts with `property_id` | every row except pooled accounts, which look like `ALLE/5` instead |

<Warning>
  Use the prefixes to filter or group by property, never to parse a key apart. Three reasons:
  segment widths are not fixed — they differ per dataset and even within a single property,
  where the same property carries both `45111` and `451110` as `floor_area_id`; prefix matching
  is therefore ambiguous, since `45111` is itself a prefix of `451110`; and building the key
  drops separators, so a `person_id` of `"010-1"` ends up as the suffix `0101` and cannot be
  split back out.

  Every component is its own field in the response. Read `property_id`, `floor_area_id`, and
  `person_id` directly instead of slicing the composite.
</Warning>
