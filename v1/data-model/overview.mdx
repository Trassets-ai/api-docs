...
month. A cost-centre account shares its identifier with `property_structure.account_id` — in
`accounts.account_number`, not `accounts.account_id`. Debit schedules (`contract_debits`,
`contract_debit_diffs`) reference the general ledger account they post to via `ledger_id`,
again matching `accounts.account_number`.
