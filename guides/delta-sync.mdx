...
Discover which tables have been updated since your last sync without fetching every endpoint. The Data API `GET /changes` endpoint returns metadata for changed tables and supports conditional requests via ETag to skip polling when nothing is new.
