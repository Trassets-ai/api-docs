---
title: "Requests an die Trassets Data API v1 authentifizieren"
sidebarTitle: "Authentifizierung"
description: "Erfahre, wie du API-Keys für die Trassets Data API beantragst und nutzt, und verstehe die möglichen Authentifizierungsfehler unter /v1/."
---

Die Trassets Data API nutzt ein einfaches API-Key-Schema. Du übergibst deinen Key im `X-API-Key`-Header bei jedem Request. Der Key bestimmt, auf welchen Kunden-Scope und welche Datenstufen du zugreifen kannst — das Schema ist unter `/v1/` identisch zur unversionierten API.
...
Füge den `X-API-Key`-Header bei jedem Call hinzu. Hier ein minimales curl-Beispiel gegen eine live `/v1/`-Tabelle:
