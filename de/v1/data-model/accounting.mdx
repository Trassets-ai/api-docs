...
<Note>
  `nr` ist `null` bei Konten, die noch auf der älteren Kontenbereichs-Klassifizierung
  laufen — diese liefert dieses Feld nicht. `category` und `group` sind in dem Fall
  trotzdem befüllt, nur anders hergeleitet.
</Note>
