...
<Note>
  `person_id` ist nicht über alle Endpunkte hinweg typkonsistent: `booking_header.person_id`
  und `Stakeholder.tenants.person_id` sind Strings (z. B. `"2001-1"`), `booking_positions.person_id`
  dagegen ein Integer, bei dem der Bindestrich beim Cast entfernt wurde — ein Rück-Cast auf
  String liefert nicht den ursprünglichen Wert. `booking_positions` deshalb nicht direkt über
  `person_id` mit `tenants` verknüpfen, sondern über `booking_positions.booking_header_id` →
