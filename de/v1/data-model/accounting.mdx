...
`booking_positions` führt ebenfalls ein `person_id`-Feld, aber als `integer` statt als
`string` wie bei `booking_header.person_id` und `tenants.person_id` — eine direkte
Verknüpfung zu `tenants` funktioniert damit nicht. Um eine Position einem Mieter
zuzuordnen, geht der Weg über `booking_positions.booking_header_id` zu
