...
<ResponseField name="afa_validity" type="string">
  Gültigkeitsdatum der AfA-Konfiguration, die für diese Anlage angewendet wird.
</ResponseField>

<Note>
  `afa_validity` ist nicht dasselbe wie `afa_period.start`. `afa_period.start` ist der
  Beginn des Abschreibungszeitraums dieser konkreten Anlage; `afa_validity` ist der
  Zeitpunkt, ab dem die verwendete AfA-Konfiguration gültig wurde. Beide können voneinander
  abweichen.
</Note>
