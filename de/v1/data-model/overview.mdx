...
| `property_structure_account_id` | Bedingter Fallback-Fremdschlüssel auf `Property.property_structure` (über den unversionierten Pfad verfügbar), vorhanden auf `booking_header`, `booking_positions` und `technical_objects`. Nur befüllt, wenn der primäre Join-Key der Tabelle die Zeile nicht abdeckt — die genaue Bedingung steht in der jeweiligen Feldreferenz. | variiert pro Tabelle, siehe Feldreferenz |
