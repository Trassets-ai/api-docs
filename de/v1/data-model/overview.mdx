...
`contract_debit_diffs` referenzieren beide das Sachkonto, auf das sie buchen — als
`ledger_id` beziehungsweise `account_number` — ebenfalls gegen `accounts.account_number`.
`contract_debits.debit_type.code` ist etwas anderes, ein Sollart-Code, kein Kontenbezug
(siehe die `account_id`-Zeile weiter unten).
...
| `ledger_id` | Sachkonto, auf das eine Sollstellungszeile bucht. Heißt bei `contract_debits` `ledger_id`, bei `contract_debit_diffs` `account_number` — gleicher Bezug, anderer Feldname. Entspricht `accounts.account_number`, nicht `accounts.account_id`. | 1 `accounts`-Zeile → viele `contract_debits`/`contract_debit_diffs`-Zeilen |
