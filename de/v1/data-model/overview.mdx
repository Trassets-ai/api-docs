...
| `tenancy_id` | Identifiziert ein Mietverhältnis. `tenants`, `contract_debits` und `contract_ends` teilen denselben Wert — direkt darüber verknüpfen, ohne `property_id`. | 1 Mietverhältnis → viele Soll-/Ende-Zeilen |
...
<Warning>
  ...
  `booking_positions` dagegen als Zahl. Vor einem Join über diese Grenze hinweg auf einen
  gemeinsamen Typ casten — ein strikter Gleichheitsvergleich auf den Rohwerten liefert
  stillschweigend keine Zeilen. `floor_area_id` hat dasselbe Problem umgekehrt: überall eine
  Zahl, außer bei `contract_debits`, wo es ein String ist.
</Warning>
