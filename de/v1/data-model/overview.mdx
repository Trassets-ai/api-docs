...
Konto-Zeile — nicht deren `accounts.account_id`. `contract_debits` referenziert das
Sachkonto, auf das es bucht, über `ledger_id` — ebenfalls gegen `accounts.account_number`.
`contract_debit_diffs.
account_number` ist etwas anderes, ein Sollart-Code, kein Kontenbezug (siehe die
`account_id`-Zeile weiter unten).
...
| `account_id` | Interne Konto-ID bei `accounts`, `booking_positions` und `budgets` — darüber verknüpfbar. `index_rent.account_id` und `contract_debits.debit_type.code` bedeuten etwas anderes: den Sollart-Code — nicht mit `accounts` verknüpfen. `contract_debit_diffs.account_number` entspricht demselben Sollart-Code unter anderem Feldnamen. Bei `property_structure` bedeutet es wieder etwas anderes: einen Kostenstellen-Geschäftscode, der `accounts.account_number` der zugehörigen Kostenstellen-Zeile entspricht, nicht `accounts.account_id`. | 1 `accounts`-Zeile → viele `booking_positions`/`budgets`-Zeilen |
| `ledger_id` | Sachkonto, auf das eine Sollstellungszeile bucht, bei `contract_debits`. Entspricht `accounts.account_number`, nicht `accounts.account_id`. | 1 `accounts`-Zeile → viele `contract_debits`-Zeilen |
