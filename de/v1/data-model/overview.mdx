...
<CardGroup cols={2}>
  ...
  <Card title="Contracts" icon="file-contract" href="/de/v1/data-model/contracts">
    `service_contracts`, mit dem `contract_period`-Objekt.
  </Card>
  ...
  <Card title="Property" icon="building" href="/de/v1/data-model/property">
    `properties`, `property_creditor_services` und `technical_objects`, mit dem
    `entrances`-Array, dem `mandate_period`-Objekt und dem bedingten
    `property_structure_account_id`-Fallback.
  </Card>

  <Card title="Stakeholder" icon="users" href="/de/v1/data-model/stakeholder">
    `creditor` und `tenants`, beide mit einem `phones`-Array.
  </Card>
</CardGroup>
...
<Note>
  ...
  nicht), `Operations.offers`/`orders`, `Property.property_structure` und
  `Stakeholder.employees`/`owner`. Bis dahin über den unversionierten Pfad abrufen — siehe
  das [vollständige Datenmodell](/de/data-model/overview).
</Note>
...
| `property_structure_account_id` | Fremdschlüssel auf `Property.property_structure` (über den unversionierten Pfad verfügbar), vorhanden auf `booking_header`, `booking_positions` und `technical_objects`. Auf den beiden Buchungstabellen immer befüllt und dort der einzige Weg von einer Buchung zu einer Einheit; auf `technical_objects` der Fallback für Zeilen ohne `floor_area_id`. | 1 Einheit → viele Zeilen |
