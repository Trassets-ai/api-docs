...
<CardGroup cols={2}>
  ...
  <Card title="Stakeholder" icon="users" href="/de/v1/data-model/stakeholder">
    `tenants`, mit Mietvertrags-Datumsfeldern und Kontaktdaten — die Spalten
    `phone_1`–`phone_3` sind noch nicht zu einem Array zusammengefasst.
  </Card>
</CardGroup>

<Note>
  `Property.properties` ist ebenfalls live unter `/v1/`, wird aber unverändert (inklusive
  der durchnummerierten Spalten `entrance_1`–`entrance_5`) statt umgeformt ausgeliefert —
  bewusst nicht Teil dieses Datenmodells. Nutze stattdessen das unversionierte
  [`Property.properties`](/de/data-model/property#properties). Jede andere Tabelle
  (`orders`, `property_structure`, `assets`, `contract_debits` usw.) ist noch nicht live
  unter `/v1/` und bleibt auf dem unversionierten Pfad.
</Note>
