...
jedem Konto kann es ein `budget` für einen bestimmten Monat geben. Ein Kostenstellen-Konto
teilt seinen Bezeichner mit `property_structure.account_id` — in `accounts.account_number`,
nicht in `accounts.account_id`. `contract_debits` referenziert das Sachkonto, auf das es
bucht, über `ledger_id` — ebenfalls gegen `accounts.account_number`. `contract_debit_diffs.
