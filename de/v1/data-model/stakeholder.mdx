...
<Note>
  `person_id` ist nicht über alle Endpunkte hinweg typkonsistent, die es führen:
  `tenants.person_id` und `Accounting.booking_header.person_id` sind Strings (z. B.
  `"2001-1"`), `Accounting.booking_positions.person_id` dagegen ein Integer, bei dem der
  Bindestrich beim Cast entfernt wurde — ein Rück-Cast auf String liefert nicht den
  ursprünglichen Wert. `booking_positions` deshalb nicht direkt über `person_id` mit
  `tenants` verknüpfen, sondern über `booking_positions.booking_header_id` →
  `booking_header.person_id`.
</Note>
...
<ResponseField name="exemption_certificate_date" type="date">
  Datum auf der Freistellungsbescheinigung.
</ResponseField>
...
<ResponseField name="industry_info" type="string">
  Zusätzliche Brancheninformation.
</ResponseField>
...
<ResponseField name="floor_area_person_id" type="string" required>
  ...
  [Datenmodell-Übersicht](/de/v1/data-model/overview#verknüpfungsschlüssel-im-detail).
</ResponseField>
