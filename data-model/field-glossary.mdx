---
title: "Field Glossary for the Data API"
sidebarTitle: "Field Glossary"
description: "Reference guide to common field names across the Data API, including entrance, address, floor_area_id, property_id, and pagination cursors."
---

This glossary defines field names that appear across multiple endpoints in the Data API. Use it as a quick reference when mapping API responses to your own data models or when designing joins across domains.
...
| `contract_end_id` | string | Internal hash identifying a specific lease-end/option record in `Contracts.contract_ends`. Not stable long-term — changes if the option deadline or contract end date is shifted. |
...
<Note>
  Fields prefixed with `sk_` are internal surrogate keys. They are not stable identifiers for external use and should not be relied on in your integrations.
</Note>
